...
static NodeIndexMap *createNodeIndexMap(GraphVtab *pVtab){
  ...
  pMap = sqlite3_api->malloc(sizeof(*pMap));
  if( pMap==0 ) return 0;

  pMap->nNodes = pVtab->nNodes;
  pMap->aNodeIds = sqlite3_api->malloc(sizeof(sqlite3_int64) * pMap->nNodes);
  if( pMap->aNodeIds==0 ){
    sqlite3_api->free(pMap);
    return 0;
  }
...
static void freeNodeIndexMap(NodeIndexMap *pMap){
  if( pMap ){
    sqlite3_api->free(pMap->aNodeIds);
    sqlite3_api->free(pMap);
  }
}
...
static BrandesState *brandesInitState(GraphVtab *pVtab,
  ...
  pState = sqlite3_api->malloc(sizeof(*pState));
  ...
  pState->aDistance = sqlite3_api->malloc(sizeof(double) * nNodes);
  pState->aSigma = sqlite3_api->malloc(sizeof(int) * nNodes);
  pState->aDelta = sqlite3_api->malloc(sizeof(double) * nNodes);
  pState->aPred = sqlite3_api->malloc(sizeof(PredecessorList*) * nNodes);

  if( pState->aDistance==0 || pState->aSigma==0 ||
      pState->aDelta==0 || pState->aPred==0 ){
    sqlite3_api->free(pState->aDistance);
    sqlite3_api->free(pState->aSigma);
    sqlite3_api->free(pState->aDelta);
    sqlite3_api->free(pState->aPred);
    sqlite3_api->free(pState);
    return 0;
  }
...
static int brandesAddPredecessor(BrandesState *pState, int iNodeIdx,
  ...
  pPred = sqlite3_api->malloc(sizeof(*pPred));
  if( pPred==0 ) return 7;
...
static BrandesQueue *brandesQueueCreate(int nCapacity){
  ...
  pQueue = sqlite3_api->malloc(sizeof(*pQueue));
  if( pQueue==0 ) return 0;

  pQueue->aItems = sqlite3_api->malloc(sizeof(int) * nCapacity);
  if( pQueue->aItems==0 ){
    sqlite3_api->free(pQueue);
    return 0;
  }
...
static void brandesQueueFree(BrandesQueue *pQueue){
  if( pQueue ){
    sqlite3_api->free(pQueue->aItems);
    sqlite3_api->free(pQueue);
  }
}
...
int graphBetweennessCentrality(GraphVtab *pVtab, char **pzResults){
  ...
  aCentrality = sqlite3_api->malloc(sizeof(double) * nNodes);
  if( aCentrality==0 ){
  ...
  for( pNode = pVtab->pNodeList; pNode; pNode = pNode->pNext ){
    ...
    while( !brandesQueueIsEmpty(pQueue) ){
      ...
      BrandesStack *pStackNode = sqlite3_api->malloc(sizeof(*pStackNode));
      if( pStackNode ){
  ...
  zResults = sqlite3_api->mprintf("{");
  for( i = 0; i < nNodes && zResults; i++ ){
    char *zOld = zResults;
    zResults = sqlite3_api->mprintf("%s%s\"%lld\":%.6f", zOld,
                              i > 0 ? "," : "",
                              pMap->aNodeIds[i],
                              aCentrality[i] / 2.0);
    sqlite3_api->free(zOld);
  }

  if( zResults ){
    char *zOld = zResults;
    zResults = sqlite3_api->mprintf("%s}", zOld);
    sqlite3_api->free(zOld);
  }
...
int graphTopologicalSort(GraphVtab *pVtab, char **pzOrder){
  ...
  aInDegree = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  aQueue = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  ...
  while( nFront < nRear && zOrder ){
    ...
    char *zOld = zOrder;
    zOrder = sqlite3_api->mprintf("%s%s%lld", zOld,
                            nProcessed > 0 ? "," : "",
                            iCurId);
    sqlite3_api->free(zOld);
    nProcessed++;
  ...
  if( nProcessed < pVtab->nNodes ){
    sqlite3_api->free(zOrder);
    rc = 19;
    *pzOrder = sqlite3_api->mprintf("Graph contains cycles");
  } else if( zOrder ){
    char *zOld = zOrder;
    zOrder = sqlite3_api->mprintf("%s]", zOld);
    sqlite3_api->free(zOld);
    *pzOrder = zOrder;
    if( zOrder==0 ) rc = 7;
  } else {
...
topo_cleanup:
  sqlite3_api->free(aInDegree);
  sqlite3_api->free(aQueue);
  freeNodeIndexMap(pMap);
  return rc;
}
...
int graphHasCycle(GraphVtab *pVtab){
  ...
  aColor = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  if( aColor==0 ){
    freeNodeIndexMap(pMap);
    return 0;
  }
...
int graphConnectedComponents(GraphVtab *pVtab, char **pzComponents){
  ...
  if( pVtab->nNodes == 0 ){
    *pzComponents = sqlite3_api->mprintf("{}");
  ...
  aComponent = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  if( aComponent==0 ){
    freeNodeIndexMap(pMap);
    return 7;
  }
  ...
  for( i = 0; i < pVtab->nNodes; i++ ){
    if( aComponent[i] == -1 ){
      ...
      int *aStack = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
      int nStack = 0;
      ...
      sqlite3_api->free(aStack);
      nComponents++;
    }
  }


  zComponents = sqlite3_api->mprintf("{");
  for( int comp = 0; comp < nComponents && zComponents; comp++ ){
    char *zOld = zComponents;
    zComponents = sqlite3_api->mprintf("%s%s\"%d\":[", zOld,
                                 comp > 0 ? "," : "", comp);
    sqlite3_api->free(zOld);
    ...
    for( i = 0; i < pVtab->nNodes && zComponents; i++ ){
      if( aComponent[i] == comp ){
        zOld = zComponents;
        zComponents = sqlite3_api->mprintf("%s%s%lld", zOld,
                                     first ? "" : ",",
                                     pMap->aNodeIds[i]);
        sqlite3_api->free(zOld);
        first = 0;
    ...
    if( zComponents ){
      zOld = zComponents;
      zComponents = sqlite3_api->mprintf("%s]", zOld);
      sqlite3_api->free(zOld);
    }
  }
...
static void tarjanStrongConnect(GraphVtab *pVtab, TarjanState *pState,
  ...
  pStackNode = sqlite3_api->malloc(sizeof(*pStackNode));
  if( pStackNode ){
  ...
  if( pState->aLowLink[iNodeIdx] == pState->aIndex[iNodeIdx] ){
    ...
    while( pState->pStack ){
      ...
      if( zSCC ){
        char *zOld = zSCC;
        zSCC = sqlite3_api->mprintf("%s%s%lld", zOld,
                              first ? "" : ",",
      ...
      sqlite3_api->free(pState->pStack);
      pState->pStack = pNext;
    ...
    if( zSCC ){
      char *zOld = zSCC;
      zSCC = sqlite3_api->mprintf("%s]", zOld);
      sqlite3_api->free(zOld);


      char **azNew = sqlite3_api->realloc(pState->azSCC,
                                    sizeof(char*) * (pState->nSCC + 1));
      if( azNew ){
        pState->azSCC = azNew;
        pState->azSCC[pState->nSCC++] = zSCC;
      } else {
        sqlite3_api->free(zSCC);
      }
    }
  }
}
...
int graphStronglyConnectedComponents(GraphVtab *pVtab, char **pzSCC){
  ...
  state.aIndex = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  state.aLowLink = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  state.aOnStack = sqlite3_api->malloc(sizeof(int) * pVtab->nNodes);
  ...
  zResult = sqlite3_api->mprintf("[");
  for( i = 0; i < state.nSCC && zResult; i++ ){
    char *zOld = zResult;
    zResult = sqlite3_api->mprintf("%s%s%s", zOld,
                             i > 0 ? "," : "",
                             state.azSCC[i]);
    sqlite3_api->free(zOld);
  }
  ...
  if( zResult ){
    char *zOld = zResult;
    zResult = sqlite3_api->mprintf("%s]", zOld);
    sqlite3_api->free(zOld);
  }
...
scc_cleanup:
  ...
  for( i = 0; i < state.nSCC; i++ ){
    sqlite3_api->free(state.azSCC[i]);
  }
  sqlite3_api->free(state.azSCC);


  while( state.pStack ){
    TarjanNode *pNext = state.pStack->pNext;
    sqlite3_api->free(state.pStack);
    state.pStack = pNext;
  }

  sqlite3_api->free(state.aIndex);
  sqlite3_api->free(state.aLowLink);
  sqlite3_api->free(state.aOnStack);
