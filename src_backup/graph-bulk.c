...
static int batchFlush(BatchAccumulator *batch, GraphVtab *pGraph) {
    ...
    for (int i = 0; i < batch->nNodes; i++) {
        ...
        /* Add to graph without indexing */
        node->pNext = pGraph->pNodeList;
        pGraph->pNodeList = node;
        pGraph->nNodes++;
    ...
    for (int i = 0; i < batch->nEdges; i++) {
        ...
        /* Add to graph */
        edge->pNext = pGraph->pEdgeList;
        pGraph->pEdgeList = edge;
        pGraph->nEdges++;
...
static void bulkLoadFunc(
    ...
    /* Would look up graph virtual table by name */
    GraphVtab *pGraph = NULL; /* Placeholder */
