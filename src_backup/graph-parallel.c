...
static void parallelPatternWorker(void *arg) {
    ...
    /* Perform pattern matching in assigned range */
    GraphNode *current = match->pGraph->pNodeList;
    sqlite3_int64 nodeCount = 0;
    
    while (current && nodeCount < match->endNode) {
        if (nodeCount >= match->startNode) {
            /* Check if node matches pattern */
            int matches = 1;
            
            if (match->pattern->zValue && current->azLabels) {
                matches = 0;
                for (int i = 0; i < current->nLabels; i++) {
                    if (strcmp(current->azLabels[i], 
                              match->pattern->zValue) == 0) {
                        matches = 1;
                        break;
                    }
                }
...
int graphParallelPatternMatch(GraphVtab *pGraph, CypherAst *pattern,
    ...
    /* Create task scheduler */
    TaskScheduler *scheduler = graphCreateTaskScheduler(0); /* Use default threads */
    ...
    sqlite3_int64 nodesPerThread = pGraph->nNodes / nThreads;
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        matches[i].endNode = (i == nThreads - 1) ? 
            pGraph->nNodes : (i + 1) * nodesPerThread;
    ...
    /* Execute in parallel */
    int rc = graphExecuteParallel(scheduler, parallelPatternWorker, 
                                 args, nThreads);
...
*/
static void parallelAggregationWorker(void *arg) {
    ParallelAggregation *agg = (ParallelAggregation*)arg;
    
    double localSum = 0;
    sqlite3_int64 localCount = 0;
    double localMin = 1e9;
    double localMax = -1e9;
    
    /* Process nodes in this worker's range */
    GraphNode *node = agg->pGraph->pNodeList;
    while (node) {
        if (node->zProperties) {
            /* Extract property value (simplified) */
            double value = 0;
            /* TODO: Parse JSON to get actual property value */
    ...
    agg->sum += localSum;
    agg->count += localCount;
    if (localMin < agg->min) agg->min = localMin;
    if (localMax > agg->max) agg->max = localMax;
